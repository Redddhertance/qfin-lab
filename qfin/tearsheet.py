...
def chart_monthly_heatmap(pnl):
    ...
    ax.set_xticklabels([labels[c - 1] for c in grid.columns], fontsize=8.5)
...
def chart_distribution(pnl):
    ...
    ax.hist(r.to_numpy() * 100.0, bins=60, color=BENCH, alpha=0.75, linewidth=0)
...
<div class="wrap">
  ...
  {perm_block}
  {notes_block}
  <footer>Generated by qfin-lab. Past performance of a backtest is not evidence
  of future returns; results are gross of financing, taxes and borrow costs
  unless stated otherwise.</footer>
</div></body></html>"""

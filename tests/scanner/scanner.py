...
#prevents terminal workingdirectory issues when running script from different locations, ensures csv file is found correctly regardless of where script is run from. this has become a more frequent issue since recent vscode updates. not sure why, implemented furhter on lines 155-157
scriptdirectory = os.path.dirname(os.path.abspath(__file__))
csvpath = os.path.join(scriptdirectory, 'scticker.csv')
tickers = pd.read_csv(csvpath)['ticker'].tolist()

#define data source and extractors for  scanner functiona
def bulk_download(ticker_list):
    ...
    if data is None or data.empty:
        print('Empty data returned from yfinance')
        return survivors
    
    if isinstance(data.columns, pd.MultiIndex):
        print('Data has MultiIndex columns, proceeding with grouped data handling.')
        available_tickers = data.columns.get_level_values(0).unique()
    else:
        print('Data does not have MultiIndex columns, unexpected format.')
        available_tickers = [ticker_list[0]] if len(ticker_list) == 1 else [] #if only one ticker, data may not be multiindex, otherwise unexpected format

    for ticker in ticker_list:
        if ticker not in available_tickers:
            continue
                
        df = data[ticker] if isinstance(data.columns, pd.MultiIndex) else data #handle both multiindex and single ticker cases
...
#additional terminal workingdirectory handling to ensure csv files are found correctly
scanner_save_path = os.path.join(scriptdirectory, 'scanner_results.csv')
risk_save_path = os.path.join(scriptdirectory, 'riskmetrics_results.csv')
